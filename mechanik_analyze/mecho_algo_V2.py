...
def main():
    ...
    while True:
        ...
        if not calibrated and (current_time - start_time) > 20:
            ...
            print("=" * 100)
            print(f"🔧 Калибровка завершена: модель {args.model}")
            ...
            if avg_fps < args.fps_threshold or avg_cpu > args.cpu_threshold:
                print(f"⚠️ Обнаружена высокая нагрузка (FPS < {args.fps_threshold} или CPU > {args.cpu_threshold:})! Переключаемся на более лёгкий детектор.")
                ...
                calibrated = True
                print("✅ Калибровка прошла успешно. Требования к FPS и CPU соблюдены.")
                cap.release()
