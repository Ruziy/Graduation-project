...
# --- монтирования под-приложений ---
app.mount("/optimizer", optimizer_app)
app.mount("/face", face_app)
app.mount("/api",  face_app)   # внешнее /api/* -> внутри face_app: /people, /known_faces

# ✅ КЛЮЧЕВОЕ: прямой WS-роут на корне, без mount
#   Вызываем ваш обработчик из faceRec_part/app.py
app.add_api_websocket_route("/ws", face_module.ws_endpoint)

# (опционально) статик из корня
