    ...
    while True:
        ...
        if not calibrated and (current_time - start_time) > 20:
            ...
            if avg_fps < args.fps_threshold or avg_cpu > args.cpu_threshold:
                calibration_log.append(f"[КАЛИБРОВКА] Активная модель: {args.model}")
                calibration_log.append(
                    f"⚠️ Обнаружена высокая нагрузка (FPS < {args.fps_threshold} или CPU > {args.cpu_threshold}%)! "
                    f"Переключаемся на более лёгкий детектор."
                )
                print("\n".join(calibration_log), flush=True)
                ...
                if args.model == "yolov8":
                    ...
                    detector = FaceAnalysis(providers=['CUDAExecutionProvider']); detector.prepare(ctx_id=0)
                elif args.model == "retinaface":
                    detector = RetinaFace()
                elif args.model == "mediapipe":
                    mp_face = mp.solutions.face_detection
                ...
                print(calibration_result, flush=True)
