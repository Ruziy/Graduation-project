...
def run_calibration_script(model="insightface", fps_threshold=10.0, cpu_threshold=50.0):
    try:
        ...
        model_matches = list(re.finditer(
            r"(?:Активная модель|Модель удовлетворяющая условиям|Используемая модель)\s*:\s*([^\n\r]+)",
            filtered_output
        ))
        if model_matches:
            selected_model = model_matches[-1].group(1).strip()
            payload_model = selected_model  
        else:
            payload_model = None

        fps_all = re.findall(r"Средний FPS:\s*([0-9]+(?:\.[0-9]+)?)", filtered_output)
        cpu_all = re.findall(r"Средняя загрузка CPU:\s*([0-9]+(?:\.[0-9]+)?)%", filtered_output)
        ...
        if payload_model:
            ...
            log_path = Path(__file__).resolve().parent.parent / "faceRec_part" / "log.txt"
            log_path.parent.mkdir(parents=True, exist_ok=True)

            with open(log_path, "w", encoding="utf-8") as file:
                file.write(f"{payload_model}\n")

        if fps_all:
            payload["avg_fps"] = float(fps_all[-1])
        if cpu_all:
            payload["avg_cpu"] = float(cpu_all[-1])
